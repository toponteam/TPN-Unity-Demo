...
public class ViewInfo {
    protected class INFO {
        ...
        protected boolean usesPixel = true;
    ...
    public static void add2ParentView(final FrameLayout view, final View childView, final INFO pViewInfo, final int gravity) {
        ...
        if (pViewInfo.usesPixel) {
            layoutParams = new FrameLayout.LayoutParams(pViewInfo.mWidth, pViewInfo.mHeight);
            layoutParams.leftMargin = pViewInfo.mX;
            layoutParams.topMargin = pViewInfo.mY;
        } else {
            Context context = view.getContext();
            layoutParams = new FrameLayout.LayoutParams(CommonUtil.dip2px(context, pViewInfo.mWidth), CommonUtil.dip2px(context, pViewInfo.mHeight));
            layoutParams.leftMargin = CommonUtil.dip2px(context, pViewInfo.mX);
            layoutParams.topMargin = CommonUtil.dip2px(context, pViewInfo.mY);
        }
        if (gravity > 0) {
...
//    }
    ...
    public INFO parseINFO(String json, String name, int px, int py) throws JSONException {
        ...
        if (_jsonObject.has("usesPixel")) {
            _info.usesPixel = _jsonObject.getBoolean("usesPixel");
        }
